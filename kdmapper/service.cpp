...
bool service::RegisterAndStart(const std::wstring& driver_path) {
	...
	const std::wstring driver_name = L"VulnerableDriver";
	const std::wstring servicesPath = L"SYSTEM\\CurrentControlSet\\Services\\VulnerableDriver";
	...
	if (1 != 0xFFFFFFFF)
		RegSetKeyValueW(dservice, L"Start", NULL, REG_DWORD,  (LPCVOID)1, sizeof(1));
	RegCloseKey(dservice);
	...
	std::wstring wdriver_reg_path = L"\\Registry\\Machine\\System\\CurrentControlSett\\Services\\VulnerableDriver" ;
	UNICODE_STRING serviceStr;
	RtlInitUnicodeString(&serviceStr, wdriver_reg_path.c_str());

	Status = NtLoadDriver(&serviceStr);
	Log("[+] LoadDriver Status 0x" << std::hex << Status << std::endl);
	
	//Never should occur since kdmapper checks for "IsRunning" driver before
	if (Status != 0x0) {// STATUS_IMAGE_ALREADY_LOADED
		return true;
	}
...
bool service::StopAndRemove(const std::wstring& driver_name) {
	...
	std::wstring wdriver_reg_path = L"\\registry\\machine\\SYSTEM\\CurrentControlSet\\Services\\VulnerableDriver" ;
	UNICODE_STRING serviceStr;
	RtlInitUnicodeString(&serviceStr, wdriver_reg_path.c_str());

	HKEY driver_service;
	std::wstring servicesPath = L"RTCore64.sys";
